...
FString AProjectZGameMode::InitNewPlayer(APlayerController* NewPlayerController, const FUniqueNetIdRepl& UniqueId,
	const FString& Options, const FString& Portal)
{
	
	// FParse::Value(*Options, TEXT("Data="), OptionString);
	// Options.Split(TEXT("?"), &OptionString, nullptr, ESearchCase::IgnoreCase, ESearchDir::FromStart);

	SelectRoute = ExtractRoute(Options);
	UE_LOG(LogTemp,Error,TEXT("Options: %s"), *SelectRoute.ToString());
	//SpawnCharacter(NewPlayerController, SelectRoute);

	//ServerGameMode Set
	// UZGameInstance* ZGameInstance = Cast<UZGameInstance>(GetGameInstance());
	// if(ZGameInstance)
	// {
	// 	ZGameInstance->StartingPoint = SelectRoute;	
	// }
...
void AProjectZGameMode::SpawnCharacter(APlayerController* LocalPlayer)
{
	...
	// UE_LOG(LogTemp,Warning,TEXT("Choose Routhe: %s"),*LocalPlayer->GetPlayerState<AZPlayerState>()->SelectRoute.ToString()); 
	//
	// UZGameInstance* ZGameInstance = Cast<UZGameInstance>(GetGameInstance());
	//
	// if (ZGameInstance != nullptr)
	// {
	// 	AZPlayerState* ZPlayerState = Cast<AZPlayerState>(LocalPlayer->PlayerState);
	// 	if(ZPlayerState)
	// 	{
	// 		
	// 		UE_LOG(LogTemp,Error,TEXT("Choose Route GI : %s"), *ZGameInstance->StartingPoint.ToString());
	// 		ZPlayerState->SelectRoute = ZGameInstance->StartingPoint;
	// 		UE_LOG(LogTemp,Error,TEXT("Choose Route PS : %s"), *ZPlayerState->SelectRoute.ToString());
	//
	// 		for (APlayerStart* PlayerStart : TActorRange<APlayerStart>(GetWorld()))
	// 		{
	// 			if (PlayerStart->PlayerStartTag == ZPlayerState->SelectRoute)
	// 			{
	// 				Location = PlayerStart->GetActorLocation();
	// 				Rotation = PlayerStart->GetActorRotation();
	//
