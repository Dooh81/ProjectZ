...
void AZBicycleCharacter::CalculateBycleSpeed(float DeltaSeconds)
{
	FVector Velocity = GetVelocity(); //Character move speed (cm/s)

	float bicyclespeed = (Velocity.Size() * 36.0f) / 1000.0f;
	float ResultSpeed = roundf(bicyclespeed / 0.1f) / 10.0f;

	float DistanceTraveled = Velocity.Size() * DeltaSeconds;
	TotalDistance+= DistanceTraveled;
	
	float km = TotalDistance / 100000.f;
	float ResultKm = roundf(km / 0.01f) / 100.0f;
	
	FString Speed = FString::Printf(TEXT("%.1f"), ResultSpeed);
	FString Km = FString::Printf(TEXT("%.2f"), ResultKm);
	
	FBicycleCalcData BicycleCalcData;
	BicycleCalcData.PlayerSpeed = Speed;
	BicycleCalcData.TotalKm = Km;

	UZGameInstance* ZGameInstance = Cast<UZGameInstance>(GetGameInstance());

	if(ZGameInstance)
	{
		ZGameInstance->BicycleCalcData = BicycleCalcData;
	}

	//update dp
	GetIngameHUD();

	//UE_LOG(LogTemp,Warning,TEXT("Player Speed = %s, TotalKm = %s"), *BicycleCalcData.PlayerSpeed, *BicycleCalcData.TotalKm);
}
...
{
	// AProjectZGameMode* ZGameMode = Cast<AProjectZGameMode>(UGameplayStatics::GetGameMode(GetWorld()));
	// if(ZGameMode)
	// {
	// 	AZInGameHUD* Hud = Cast<AZInGameHUD>(ZGameMode->HUDClass);
	// 	if(Hud)
	// 	{
	// 		UE_LOG(LogTemp,Warning,TEXT("GetIngameHUD3"));
	// 		UInGameInfoWidget* IngameWidget = Cast<UInGameInfoWidget>(Hud->InGameWidget);
	// 		IngameWidget->StatusWidget->UpdateBicycleStatus();
	// 	}
	// }
